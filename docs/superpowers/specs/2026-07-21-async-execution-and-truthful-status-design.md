...
日期：2026-07-21  
状态：已实施并通过 2026-07-21 全量 Fake 门禁
...
- `Dramatizer.Workflow.Enqueue`：用 Ecto.Multi 原子锁定可入队 NodeRun、插入或复用对应 Oban Job，并把 Worker 与 job identity 写回 NodeRun；各领域 Pipeline 负责创建或复用 WorkflowRun/NodeRun 图。
...
- `Dramatizer.Generation.Pipeline` 与 `Dramatizer.Quality.Jobs.NodeRunner`：图片落盘后解锁并入队技术和语义 QC NodeRun；不再在图片持久化事务内同步运行 QC。
...
本轮同时完成了工作区当前业务所需的 queued、loading/running、failed、unknown、ready 与人工门状态映射及下一步门控；移动端检查器、完整历史分页和视觉系统重构仍属于后续阶段。
...
2026-07-21 的实现与验证结果如下：

- 分析 DAG、结构化 Proposal、图片 Prompt/生成/QC 和 Timeline 渲染均已迁移为只携带 `node_run_id` 的 Oban Worker；LiveView 入口只准备本地事实并入队。
- WorkerLifecycle、WorkerRegistry、租约、有限退避、Reconciler、项目级 Notifier 与工作区切片订阅均已落地。
- 入口拓扑与根 Job、ready child 与其 Job、节点终态与 DAG 推进分别在单一事务内提交；终态 Job 重放可以幂等修复旧的推进缺口。
- Provider/媒体暂态错误进入有限重试，确定性 QC 失败仍生成报告；RenderManifest、NodeRun 与 WorkflowRun 在重试和异常路径保持聚合一致。JobGuard 将未预期控制流转为脱敏生命周期失败；若 Provider Attempt 已提交，则进一步收敛为 `unknown_remote_state`。
- `unknown_remote_state` 用于真实 Provider 提交超时后的未知远端结果，自动重试会在该边界停止。
- LiveView 提交后显示真实排队文案；事务提交后发出最终失效通知，重挂载与 PubSub 刷新都从数据库恢复状态，重复点击复用已有执行。历史 Attempt 保留审计但不覆盖当前流程状态。
- `mix.bat format --check-formatted`、`mix.bat compile --warnings-as-errors`、`mix.bat assets.build` 均通过；2026-07-22 分析恢复、执行配置冻结与最新快照消费加固后的 `mix.bat test` 为 **183 passed、1 excluded**；Fake Chromium E2E 为 **1 passed（45.5s）**。
- E2E 的异步等待以必需参考槽位数为事实，不再在首个候选出现时误判整批已完成；完整流程验证 8 个参考槽、6 个镜头候选、预览/正式渲染、故障恢复和成本幂等。
- `scripts/real-smoke.ps1` 无费用复核最近一次真实 Provider 脱敏证据为 PASS；本轮未使用 `-Force` 重跑完整付费门禁。随后通过 4000 开发实例完成一次新的真实 OpenAI 六节点全文分析与 Narrative Proposal，用于验证 Fake/OpenAI 执行身份隔离和恢复后的实际运行路径。
