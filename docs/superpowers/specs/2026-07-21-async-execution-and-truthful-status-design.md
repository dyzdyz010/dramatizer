...
- `mix.bat format --check-formatted`、`mix.bat compile --warnings-as-errors`、`mix.bat assets.build` 均通过；`mix.bat test` 为 **166 passed、1 excluded**；Fake Chromium E2E 为 **1 passed（31.6s）**。
- E2E 的异步等待以必需参考槽位数为事实，不再在首个候选出现时误判整批已完成；完整流程验证 8 个参考槽、6 个镜头候选、预览/正式渲染、故障恢复和成本幂等。
- `scripts/real-smoke.ps1` 无费用复核最近一次真实 Provider 脱敏证据为 PASS；本轮未使用 `-Force` 产生新的付费调用。
