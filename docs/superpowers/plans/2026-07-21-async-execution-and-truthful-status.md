...
- [x] **Step 1: Make the regression deterministic and prove the old query is ambiguous**
...
- [x] **Step 2: Run the focused test repeatedly**
...
- [x] **Step 3: Run tests and confirm undefined-module failures**
...
- [x] **Step 4: Implement the two focused modules**
...
- [x] **Step 2: Run lifecycle tests and observe missing fields/modules**
...
- [x] **Step 3: Add the migration and NodeRun changeset support**
...
- [x] **Step 4: Implement lifecycle locking and deterministic backoff**
...
- [x] **Step 5: Write and run reconciler tests**
...
- [x] **Step 6: Enable periodic and manual reconciliation**
...
- [x] **Step 1: Write failing atomic-enqueue tests**
...
- [x] **Step 3: Implement Ecto.Multi enqueue**
...
- [x] **Step 1: Write failing enqueue tests**
...
- [x] **Step 2: Run focused tests and confirm synchronous behavior fails**

Run: `mix.bat test test/dramatizer/analysis/dag_test.exs`

Expected: new enqueue assertions fail before production changes.

- [x] **Step 3: Implement enqueue and single-node worker execution**
...
- [x] **Step 4: Convert acceptance tests to drain explicit Oban jobs**
...
- [x] **Step 5: Run analysis tests and commit**
...
- [x] **Step 1: Write failing pipeline topology and no-inline-Provider tests**
...
- [x] **Step 2: Run tests and observe missing pipeline API**

Run: `mix.bat test test/dramatizer/generation/pipeline_test.exs`

Expected: undefined `Dramatizer.Generation.Pipeline`.

- [x] **Step 3: Implement persisted pipeline definitions and worker dispatch**
...
- [x] **Step 4: Make QC explicit child nodes**
...
- [x] **Step 5: Test success, duplicate enqueue, retry, and unknown remote state**
...
- [x] **Step 6: Run focused suite and commit**
...
- [x] **Step 1: Write failing asynchronous render tests**
...
- [x] **Step 2: Run focused tests and observe synchronous API failure**
...
- [x] **Step 3: Implement render workflow envelope and lifecycle**
...
- [x] **Step 2: Run LiveView tests and capture synchronous failures**
...
- [x] **Step 3: Replace direct long-running calls with enqueue commands**
...
- [x] **Step 4: Subscribe and reload by resource**
...
- [x] **Step 1: Update E2E waits to durable visible states**
...
- [x] **Step 2: Run the Fake E2E and fix only demonstrated failures**

Run: `.\scripts\e2e.ps1`

Expected: `1 passed` and no listener left on the test port.

- [x] **Step 3: Update documentation from measured facts**
...
- [x] **Step 4: Run the full local verification gate**
...
- [x] **Step 5: Run explicit real-provider smoke only when credentials are present**
...
### Task 10: Independent Review Hardening

- [x] 将入口拓扑、根任务和运行态切换纳入同一个数据库事务，并用非法 Job changeset 证明失败时无部分记录残留。
- [x] 将 ready child 的状态迁移、Oban Job 插入和任务 ownership 纳入同一个事务。
- [x] 将节点终态与 DAG 推进/WorkflowRun 终态纳入同一个事务，并为终态重放增加幂等自修复测试。
- [x] 对齐真实适配器错误、语义/技术 QC 暂态错误和 `unknown_remote_state`，阻止不安全的自动重提。
- [x] 增加脱敏 JobGuard、当前工作流状态派生和事务提交后 LiveView 失效通知。
- [x] 重新运行完整 ExUnit、assets、Chromium E2E、合同校验和无费用真实证据复核。
