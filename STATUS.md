...
- WorkerLifecycle 统一执行权、job identity、租约、有限退避、终态和重复执行短路；Reconciler 每分钟检查租约过期或缺失 worker/job/租约的 running 节点，只通过 Worker registry 恢复，不从数据库字符串创建任意模块。若孤儿节点已有 submitted Attempt，则稳定为 `unknown_remote_state` 并同步终结 WorkflowRun，禁止自动重提。
...
- UI 区分 queued、running、failed、unknown、ready 与人工处理状态；阶段状态、分析审阅、分集候选和选择动作都绑定最新 Analysis WorkflowRun 及其自己的 AnalysisSnapshot。历史 Attempt 继续显示在审计面板，但历史快照不会把失败/运行中的新流程伪装成 ready，也不能继续驱动 Narrative；已挂载页面收到新 run 的 workflow 通知时会立即同步清空旧候选。“已加入队列”只表示任务已被接受，不能表示已经完成。
- 全文分析的 Provider 模式、工作流 schema 版本和每个任务的脱敏执行配置进入 Workflow 幂等身份并固化到 NodeRun；Fake 与 OpenAI 不复用同一个分析 run，worker 不读取启动后可能变化的进程模式或项目模型覆盖。Provider 请求身份不含 NodeRun 生命周期重试计数，恢复后会复用已成功 Attempt；失败节点和没有远端提交记录的孤儿节点可以事务化恢复，已提交但结果未知的节点不可自动恢复。
...
- [`docs/superpowers/specs/2026-07-22-analysis-resume-recovery-design.md`](docs/superpowers/specs/2026-07-22-analysis-resume-recovery-design.md)
- [`docs/implementation-alignment.md`](docs/implementation-alignment.md) D-061

## 2026-07-22 fresh gate

- `mix.bat format --check-formatted`：通过。
- `mix.bat compile --warnings-as-errors`：通过。
- `mix.bat test`：**183 passed，1 excluded**，退出码 0。两条 `worker execution raised` 是异常恢复用例的预期观测；测试结束阶段的 Postgrex 客户端断开是 LiveView 测试进程退出后的连接回收日志，不影响结果。
- `mix.bat assets.build`：Tailwind 与 esbuild 通过。
- `./scripts/e2e.ps1`：Chromium **1 passed（45.5s）**。覆盖独立数据库迁移、项目创建、模型覆盖、小说导入、异步六节点分析、三类异步 Proposal、8 个参考槽候选及完整主图选择、6 个镜头候选与 QC、字幕编辑、异步预览/正式渲染、MP4/SRT 下载与 FFprobe、提交后状态刷新、失败恢复、重复乱序回调去重、成本幂等、全阶段路由及无裸 JSON 编辑面。
- 当前 `aaa` 项目已完成现场复验：旧 OpenAI 孤儿 run 被安全归一化为 failed，两个遗留 submitted Attempt 变为 `unknown_remote_state`；新的 Fake 执行身份先完成 6 个 AnalysisNodeJob，随后切换至 OpenAI 后又以独立执行身份完成真实六节点全文分析和 Narrative Proposal。最新两个 WorkflowRun 均 succeeded，共记录 13 个 Attempt、11 个 Provider request ID 和 553,811 usage units；页面生成 9 个分集候选并停在人工选择边界，当前没有内容任务排队。
- v0.2 合同校验：**8 schemas、7 mapped examples、33 negative cases** 与全部本地 Markdown 链接通过。
- `./scripts/real-smoke.ps1`：**PASS（复用最近一次成功的真实 Provider 脱敏证据，脚本本身未产生新费用）**；证据包含 6 个分析节点、8 张参考图、6 张镜头候选、14 份技术 QC、14 份语义 QC、3 个 Clip、49 个请求快照、48 个 Provider request ID、220,027 usage units 和 1080×1920 正式视频。此次异步重构后未使用 `-Force` 重跑完整付费门禁；另已通过 4000 开发实例完成一次新的真实 OpenAI 全文分析与 Narrative Proposal，证据见上一条。
...
- 当前根目录 `.env` 配置为 `openai`，4000 预览实例也已按该配置运行；网页中的分析、Proposal、图片生成和语义 QC 操作会真实调用 OpenAI 并可能计费。普通测试与 E2E 仍显式强制 Fake。
...
- 4000 端口当前运行修复后的 OpenAI 预览实例；E2E 使用的 4100 临时服务已由脚本 `finally` 正常回收。
